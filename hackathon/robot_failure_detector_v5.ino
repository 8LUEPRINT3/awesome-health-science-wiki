...
void setup() {
  ...
  Serial.println("# JOINTIQ v5.0 — 4x DHT11 + 3x Piezo (no pot)");
  Serial.println("# FORMAT: DATA|ZONE|DERIVED_LOAD|T1|T2|T3|T4|P1|P2|P3|FAULT");
}
...
void loop() {
  ...
  // ── Derived load — proxy from piezo + temp average ──
  // Read raw piezos for load proxy BEFORE window resets
  float derivedLoad = 0;

  // ── Baseline learning ──
  if (!blLocked) {
    blSamples++;
    blSum += 50.0; // neutral baseline while learning
    if (now - blStart >= BL_LEARN_MS) {
      baseline = 50.0;
      blLocked = true;
      lcd.clear();
      lcd.setCursor(0,0); lcd.print("JOINTIQ  READY");
      lcd.setCursor(0,1); lcd.print("4 TEMP + 3 PIZ");
      Serial.println("# Baseline locked — 4xDHT11 + 3xPiezo");
    }
    delay(50);
    return;
  }

  // Derive load from highest piezo reading (0–100% scale)
  int maxPiezo = max(max(piezoMax1, piezoMax2), piezoMax3);
  derivedLoad = (maxPiezo / 1023.0) * 100.0;
  float smoothLoad = derivedLoad; // alias for serial output
  ...
  // Stall detection (IR-only, no load threshold needed)
  if((now - lastIR) > STALL_MS){
    faulted = true;
